...
- [ ] 1.1 Build forge-ffi dylib (prerequisite)
  ...
  - **Files**: `~/gpu_kernel/metal-forge-compute/Cargo.toml`, `/Users/patrickkavanagh/pipes/libforge_ffi.dylib` (copy)
...
- [ ] 1.2 Create forge_bindings.py -- ctypes wrapper
  ...
  - **Files**: `/Users/patrickkavanagh/pipes/forge_bindings.py`
...
- [ ] 1.3 Create gpu_solver.py -- Metal kernel source strings
  ...
  - **Files**: `/Users/patrickkavanagh/pipes/gpu_solver.py`
  ...
  - **Verify**: Kernels compile: `python3 -c "from forge_bindings import ForgeContext, ForgeKernel; from gpu_solver import GEN_PATTERNS_3_MSL; ctx = ForgeContext(); k = ForgeKernel(ctx, GEN_PATTERNS_3_MSL, 'gen_patterns_3'); print('Kernel compiled OK')"`
  - **Commit**: `feat(gpu): add Metal kernel source strings for pattern generation`
  - _Requirements: FR-2_
  - _Design: Metal Kernel Design_

- [ ] 1.4 Create gpu_solver.py -- GPUPatternGenerator class
  ...
  - **Files**: `/Users/patrickkavanagh/pipes/gpu_solver.py`
  - **Done when**: Can generate patterns from sample data: `python3 -c "from gpu_solver import GPUPatternGenerator; from forge_bindings import ForgeContext; ..."` runs without error
  - **Verify**: `python3 -c "from pipe_optimizer_v5_safe import load_pipe_data; from collections import Counter; import numpy as np; d = load_pipe_data('data/sample_pipe_data.csv'); r = [round(l, 1) for l in d.lengths]; inv = Counter(r); ul = sorted(inv.keys(), reverse=True); from forge_bindings import ForgeContext; from gpu_solver import GPUPatternGenerator; ctx = ForgeContext(); g = GPUPatternGenerator(ctx, ul, inv, 100.0, 5.0); p = g.generate(3); print(f'{len(p)} patterns generated')"` -- should print a count close to CPU pattern count
  - **Commit**: `feat(gpu): add GPUPatternGenerator class with Metal kernel dispatch`
  - _Requirements: FR-3_
  - _Design: Component B, Data Flow_

- [ ] 1.5 Create gpu_solver.py -- greedy_solve function
  ...
  - **Verify**: Run greedy on sample data, check pile count is reasonable (>= 250 for 264 optimal)
  - **Commit**: `feat(gpu): add greedy_solve function for fast solving`
  - _Requirements: FR-4_
  - _Design: Component B, Data Flow_

- [ ] 1.6 Create gpu_solver.py -- GPUPipeOptimizer class
  ...
  - **Verify**: `python3 -c "from gpu_solver import GPUPipeOptimizer; from pipe_optimizer_v5_safe import load_pipe_data, export_to_excel; d = load_pipe_data('data/sample_pipe_data.csv'); opt = GPUPipeOptimizer(d.lengths, 100.0, 5.0, 1); p = opt.generate_patterns(3); s, st, t = opt.solve_greedy(p); print(f'{len(s)} piles in {t:.2f}s ({st})'); export_to_excel(opt, s, '/tmp/gpu_test.xlsx')"` -- produces valid Excel
...
- [ ] 1.7 POC Checkpoint -- end-to-end GPU pattern gen + greedy solve
  ...
  - **Verify**: Manual comparison: GPU patterns ~= CPU patterns (within 5%), greedy piles >= 250, Excel has 3 sheets
...
- [ ] 2.1 Add lp_guided_solve function
...
- [ ] 2.2 Add CPU fallback for pattern generation
...
- [ ] 2.3 Modify pipe_optimizer_gui.py -- add solver mode dropdown + worker routing
  ...
  - **Files**: `/Users/patrickkavanagh/pipes/pipe_optimizer_gui.py`
  - **Done when**: GUI launches with solver mode dropdown, all 3 modes selectable
  - **Verify**: `python3 -c "import pipe_optimizer_gui"` imports without error. Manual test: launch GUI, select each solver mode.
...
- [ ] 3.1 Create test_gpu_solver.py -- all validation tests
  ...
  - **Files**: `/Users/patrickkavanagh/pipes/test_gpu_solver.py`
  - **Done when**: `python3 -m pytest test_gpu_solver.py -v` or `python3 test_gpu_solver.py` runs all 6 tests
  - **Verify**: `cd /Users/patrickkavanagh/pipes && python3 -m pytest test_gpu_solver.py -v` -- all tests pass
...
- [ ] 4.1 Local quality check
  ...
  - **Verify**: All checks pass, no import errors, no runtime errors
